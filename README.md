# Tauros Procedural World Generator

![Version](https://img.shields.io/badge/version-1.2.0-blue)
![Python](https://img.shields.io/badge/python-3.10%2B-green)
![PySide6](https://img.shields.io/badge/PySide6-supported-yellow)
![License](https://img.shields.io/badge/license-MIT-lightgrey)

Tauros es un generador procedural de mapas simple escrito en Python.  
Está pensado como un generador para uso personal (mapas para juegos, DND, etc.) totalmente diseñado en Python.

## Features actuales (v1.2.0)
- Generación de mapas grandes usando Perlin noise.
- Generación de mapas de temperatura, humedad y biomas.
- Interfaz PySide6 para visualizar mapas.
- Guardado de configuraciones.

## 📦 Instalación
```bash
git clone https://github.com/agustin-russo/tauros.git
cd TAUROS
pip install -r requirements.txt
